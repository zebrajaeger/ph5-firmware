...
class ESPAsync_WiFiManager_Lite
{
  public:
    ...
    void connectWiFi(const char* ssid, const char* pass)
    {
      ...
      Serial.println("!!!! WiFi.mode(WIFI_STA) (A)");
      WiFi.mode(WIFI_STA);
...
#define TIMEOUT_CONNECT_WIFI      30000
      ...
      Serial.println("!!!! WiFi.mode(WIFI_STA) (B)");
      WiFi.mode(WIFI_STA);
      ...
      Serial.print("[ZJ] hadConfigData: ");
      Serial.println(hadConfigData);
      Serial.print("[ZJ] noConfigPortal: ");
      Serial.println(noConfigPortal);
      Serial.print("[ZJ] isForcedConfigPortal: ");
      Serial.println(isForcedConfigPortal);
      

      if (hadConfigData && noConfigPortal && (!isForcedConfigPortal) )
      {
        Serial.println("[ZJ] (A) STA mode ");
        hadConfigData = true;
        ...
        {
          Serial.println("[ZJ] (B) STA mode ");
          ESP_WML_LOGINFO(F("bg: WiFi OK."));
        }
        else
        {
          Serial.println("[ZJ] (B) AP mode ");
      ...
      else
      {
        ...
        {
          ESP_WML_LOGINFO1(F("bg:Stay forever in CP (A):"),
                           isForcedConfigPortal ? F("Forced-Persistent") : (noConfigPortal ? F("No ConfigDat") : F("DRD/MRD")));
        }
        else
        {
          ESP_WML_LOGINFO1(F("bg:Stay forever in CP (B):"),
...
#define WIFI_STATUS_CHECK_INTERVAL    5000L
  ...
  //  Serial.print("[ZJ] configuration_mode (X): ");
  //     Serial.println(configuration_mode);
  //     Serial.print("[ZJ] (curMillis > checkstatus_timeout) (X): ");
  //     Serial.println((curMillis > checkstatus_timeout));
      ...
      else if (configuration_mode)
      {
        ...
        Serial.println("[ZJ] configuration_mode = false (A)");
...
Serial.println("!!!! WiFi.mode(WIFI_STA) (C)");
      WiFi.mode(WIFI_STA);
  ...
  #define CONFIG_TIMEOUT      60000L
    ...
    void startConfigurationMode()
    {
      ...
      // We need a portal withount password
      ...
      Serial.println("!!!! WiFi.mode(WIFI_AP) (D)");
      WiFi.mode(WIFI_AP);
      ...
      WiFi.softAP(portal_ssid.c_str(), NULL, channel);
