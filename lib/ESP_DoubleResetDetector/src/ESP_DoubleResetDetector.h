...
#define DRD_SERIAL Serial
// #define DRD_SERIAL serialAndTelnet
...
class DoubleResetDetector
{
  public:
...
#if ESP_DRD_USE_EEPROM
#if (DOUBLERESETDETECTOR_DEBUG)
      DRD_SERIAL.printf("EEPROM size = %d, start = %d\n", EEPROM_SIZE, EEPROM_START);
#endif
...
#if ESP_DRD_USE_LITTLEFS
        DRD_SERIAL.println("LittleFS failed!. Please use SPIFFS or EEPROM.");
#else
        DRD_SERIAL.println("SPIFFS failed!. Please use LittleFS or EEPROM.");
#endif
    ...
    bool detectDoubleReset()
    {
      ...
      {
#if (DOUBLERESETDETECTOR_DEBUG)
        DRD_SERIAL.println("doubleResetDetected");
#endif
      ...
      {
#if (DOUBLERESETDETECTOR_DEBUG)
        DRD_SERIAL.println("No doubleResetDetected");
#endif
    ...
    void loop()
    {
      ...
      {
#if (DOUBLERESETDETECTOR_DEBUG)
        DRD_SERIAL.println("Stop doubleResetDetecting");
#endif
  ...
  private:
    ...
    bool detectRecentlyResetFlag()
    {
...
#if (DOUBLERESETDETECTOR_DEBUG)
      DRD_SERIAL.printf("EEPROM Flag read = 0x%X\n", DOUBLERESETDETECTOR_FLAG);
#endif
#elif ( ESP_DRD_USE_LITTLEFS || ESP_DRD_USE_SPIFFS )
      ...
      if (FileFS.exists(DRD_FILENAME))
      {
        ...
        {
#if (DOUBLERESETDETECTOR_DEBUG)
          DRD_SERIAL.println("Loading config file failed");
#endif
        }
...
#if ESP_DRD_USE_LITTLEFS
        DRD_SERIAL.printf("LittleFS Flag read = 0x%X\n", DOUBLERESETDETECTOR_FLAG);
#else
        DRD_SERIAL.printf("SPIFFS Flag read = 0x%X\n", DOUBLERESETDETECTOR_FLAG);
#endif
    ...
    void setRecentlyResetFlag()
    {
      ...
      DRD_SERIAL.printf("SetFlag write = 0x%X\n", DOUBLERESETDETECTOR_FLAG);
#endif
#elif ( ESP_DRD_USE_LITTLEFS || ESP_DRD_USE_SPIFFS )
      // LittleFS / SPIFFS code
      File file = FileFS.open(DRD_FILENAME, "w");
#if (DOUBLERESETDETECTOR_DEBUG)
      DRD_SERIAL.println("Saving config file...");
#endif
      ...
      if (file)
      {
        ...
        file.close();
#if (DOUBLERESETDETECTOR_DEBUG)
        DRD_SERIAL.println("Saving config file OK");
#endif
      }
      else
      {
#if (DOUBLERESETDETECTOR_DEBUG)
        DRD_SERIAL.println("Saving config file failed");
#endif
      }
    ...
    void clearRecentlyResetFlag()
    {
      ...
      DRD_SERIAL.printf("ClearFlag write = 0x%X\n", DOUBLERESETDETECTOR_FLAG);
#endif
#elif ( ESP_DRD_USE_LITTLEFS || ESP_DRD_USE_SPIFFS )
      // LittleFS / SPIFFS code
      File file = FileFS.open(DRD_FILENAME, "w");
